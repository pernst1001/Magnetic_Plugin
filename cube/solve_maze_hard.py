...
from .create_prims import MAZE_CUBE_CFG, GOAL_CFG, TERRAIN_HARD_CFG
...
class CubeEnvCfg(DirectRLEnvCfg):
    ...
    # scene
    scene: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=1, env_spacing=8.0, replicate_physics=False)
    cube_cfg = MAZE_CUBE_CFG.replace(prim_path="/World/envs/env_.*/Cube")
    goal_cfg = GOAL_CFG
    terrain_cfg = TERRAIN_HARD_CFG#.replace(prim_path="/World/envs/env_.*/Terrain")
    #reward
    rew_scale_terminated = -4.0
    rew_scale_goal_reached = 10.0
...
class CubeEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CubeEnvCfg, **kwargs):
        ...
        self.workspace_bounds = torch.tensor([[-2.5, -2.5, 0.0], [2.5, 2.5, 1.0]], device=self.device)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        default_root_state[:, :2] += 2.1
        default_root_state[:, 2] = 0.15
        ...
        # self._goal_positions[env_ids] = (torch.rand(num_reset, 3, device=self.device) * 2 - 1)*0.25 + self.scene.env_origins[env_ids]
        self._goal_positions[env_ids] = self.scene.env_origins[env_ids]
        self._goal_positions[env_ids, :2] -= 2.2
        self._goal_positions[env_ids, 2] = 0.15
