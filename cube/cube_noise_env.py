...
class EventCfg:
    ...
    reset_root_state = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("cube", body_names=".*"),
            "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (0.1, 0.1)},  # Specify ranges for x, y, z
            "velocity_range":{
                "x": (-0.0, 0.0),
                "y": (-0.0, 0.0),
                "z": (0.0, 0.0),
...
class CubeEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions):
        ...
        # actions += (torch.rand_like(actions) * 2 -1) * self.cfg.action_noise_scale
    ...
    def _get_observations(self):
        ...
        # cube_xy += (torch.rand_like(cube_xy) * 2 - 1) * self.cfg.observation_noise_scale
